...
function NotificationItem({ type, time, title, message, amount, price, recipient, read }) {
    ...
    if (read) {
        iconSrc = type === "refund" ? "/assets/svgs/refund_deactive.svg" : "/assets/svgs/purchase_deactive.svg";
        bgColor = "bg-zinc-100";
        textColor = "";
    }
    else {
        iconSrc = type === "refund" ? "/assets/svgs/refund.svg" : "/assets/svgs/purchase.svg";
        ...
        <div className="flex gap-4 items-start mt-6 w-full">
            ...
            <div className="flex flex-col flex-1 shrink basis-5 min-w-[240px]">
                <div className="flex gap-3 items-center w-full font-medium">
                    <div className="flex-1 shrink self-stretch font-semibold my-auto text-lg tracking-tight leading-none text-black basis-0">
                        {title}
                    </div>
                ...
                <div className={`flex-1 shrink gap-3 self-stretch mt-1 w-full text-base tracking-normal leading-6 max-w-[35vh]`}>
                    {type === "refund" ? (
                        <>
                            Your gift of{" "}
                            <span className={`font-semibold tracking-normal leading-5 ${textColor}`}>
                                {amount} coins
                            </span>{" "}
                            to <span className={`font-semibold ${textColor}`}>{recipient}</span> was refunded to your coin balance
                        </>
                    ) : (
                        <>
                            You purchased{" "}
