		...
		case "media":
			return (
				<div className="w-full flex flex-col">
					<div className={`flex ${isMe ? "justify-start" : "justify-end"}`}>
						<div
							...
							{content.type === "image" ? (
								<img
									src={content.url}
									alt="Uploaded media"								loading="lazy"									className={`max-w-full h-auto rounded-t-3xl ${
					...
					<p className={`text-[14px] mt-2 ${isMe ? "text-left" : "text-right"}`}>
						{moment.duration(moment(time).diff(new Date())).humanize(true)}
					</p>
				</div>
			);
			...
			<div className="w-full flex items-center max-h-14">
				...
				{message.length > 0 ? (
					...
					<Fab color="black" onClick={() => setAttachOpen((o) => !o)}>
						<img
							className={`${attachOpen ? "rotate-45" : ""} transition-transform`}
...
export default function Page() {
	...
	return (
		<DemoChatProvider chatId={id} guest={true}>
			<DemoChatMain />
		</DemoChatProvider>
	);
}
